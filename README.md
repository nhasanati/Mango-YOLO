...
- **Duplicate check.** Cross-partition duplication was checked by computing MD5 checksums of all 468 images. MD5 is a hashing algorithm that turns a file's contents into a fixed-length digital fingerprint, so two files with identical content yield identical checksums regardless of their file names — which is what makes duplicates detectable even when the files were renamed. No duplicates were found *within* any partition, and none at all in the three-class configuration. In the four-class configuration, one evaluation image, `4-class/images/test/376c8fb7-reject_10.jpg`, is byte-identical to the training image `4-class/images/train/5827c6a2-reject_5.jpg` (the same source image was ingested twice under different identifiers during annotation; both copies were annotated independently, 4 Reject boxes each). It is kept so the released data matches exactly what the models were trained and evaluated on. Re-evaluating the released 4-class model (`models/train2/best.pt`) with that image excluded changes mAP@0.5 by less than 0.001, so its effect on the reported figures is negligible (Ultralytics 8.4.75, `imgsz=480`, default confidence/IoU):

  | Evaluation set | Images | Instances | mAP@0.5 | mAP@0.5:0.95 | Precision | Recall |
  |---|---|---|---|---|---|---|
  | full evaluation partition | 141 | 190 | 0.9534 | 0.8782 | 0.9154 | 0.8619 |
  | without `376c8fb7-reject_10.jpg` | 140 | 186 | 0.9531 | 0.8730 | 0.9142 | 0.8488 |
  | Δ | −1 | −4 | +0.0003 (< 0.001) | +0.0052 | +0.0012 | +0.0131 |
